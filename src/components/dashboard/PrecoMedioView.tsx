...
import { FUEL_CONFIG, GAS_STATIONS } from '../../data/mockData';
...
export function PrecoMedioView({
  ...
  const fuelSummary = (Object.keys(FUEL_CONFIG) as FuelType[]).map((fuel) => {
    ...
    return {
      ...
      variationPercent: prices.reduce((a, b) => a + b.variationPercent, 0) / prices.length,
      variationAmount: prices.reduce((a, b) => a + b.variationAmount, 0) / prices.length,
    };
  }).filter((f): f is NonNullable<typeof f> => f !== null && selectedFuels.includes(f.fuel));
